...
export default function Toast({ message, tone = 'success' }) {
  ...
  return (
    <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-xs glass-card border-violet-500/35 bg-slate-900/90 py-3 px-4 shadow-xl flex items-center gap-2.5 animate-slideUp">
      <div className={`w-2 h-2 rounded-full animate-ping ${tone === 'error' ? 'bg-rose-400' : 'bg-emerald-400'}`} />
      <span className="text-[11px] font-extrabold text-slate-200 tracking-tight">{message}</span>
    </div>
  );
}
